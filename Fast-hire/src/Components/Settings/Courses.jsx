...
const Courses = () => {
  ...
  const fetchCourses = async () => {
    try {
      ...
      AlertService.error("Failed to fetch courses");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    if (!formData.courseName.trim()) {
      AlertService.warning("Please enter course name");
      return;
    }
    ...
    try {
      if (editingCourse) {
        ...
        AlertService.success("Course created successfully");
      }
      fetchCourses();
      handleCloseDialog();
    } catch (error) {
      AlertService.error("Failed to save course");
    }
  };
  ...
  const handleDelete = async (id) => {
    try {
      ...
      AlertService.error("Failed to delete course");
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "courseName",
      align: "center",
      render: (text) => <Typography sx={{ fontWeight: 500 }}>{text}</Typography>,
    },
    {
      title: "Actions",
      key: "actions",
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      >
        <DialogTitle sx={{ fontWeight: 600, fontSize: "1.25rem", mb: -1, pb: 0 }}>
